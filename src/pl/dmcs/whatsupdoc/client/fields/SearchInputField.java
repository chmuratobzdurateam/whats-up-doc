...
public class SearchInputField extends Field implements FieldConstraint {
	 ...
	 * @param search - button that will iniciate searching ( must have defined onClick funtion )
	 */
	public SearchInputField(String title, SearchFieldType type, Button search){
		super(4, Arrays.asList(new String[]{"title", "input", "searchButton", "error"}));
		...
		subDiv.get(2).add(this.searchButton);
		subDiv.get(3).add(this.error);
	}
		...
		switch (this.myType) {
			case PESEL_SEARCH:
				if(FieldVerifier.isValidPESEL(this.input.getText())){
					...
					error.setText("PESEL jest nieprawidlowy");
					return false;
				}
